...
function CheckoutContent() {
  ...
  useEffect(() => {
    getProfile().then(profile => {
      if (profile) {
        setUser(profile);
        setFormData(prev => ({ ...prev, name: profile.name, phone: prev.phone, address: prev.address }));
      }
    });
  }, []);
  ...
  return (
    <div className="bg-gray-50 min-h-screen py-12">
      <div className="container mx-auto px-4 md:px-6 max-w-6xl">
        <div className="mb-8">
          <Link href="/products" className="inline-flex items-center text-sm text-gray-500 hover:text-primary transition-colors font-medium">
                    ...
                    {items.map((item) => (
                      <div key={item.product.id} className="flex gap-4">
                        <div className="w-16 h-16 bg-gray-100 rounded-xl overflow-hidden relative shrink-0">
                          <Image 
                            src={item.product.images?.[0] || "https://images.unsplash.com/photo-1622560480654-d96214fdc887"} 
                        ...
                        <div className="flex-1 flex flex-col justify-center">
                          <h3 className="font-bold text-foreground line-clamp-1 leading-tight">{item.product.name}</h3>
